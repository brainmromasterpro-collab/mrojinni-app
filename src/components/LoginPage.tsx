...
export default function LoginPage({ sinAcceso, email, onSalir }: { sinAcceso?: boolean; email?: string; onSalir?: () => void } = {}) {
      ...
      <div className="bg-brain-card border border-brain-border rounded-2xl p-10 w-full max-w-sm flex flex-col items-center gap-6 shadow-2xl">
        <div className="flex flex-col items-center gap-2">
          ...
          <p className="text-[12px] text-[#666] text-center">
            {sinAcceso ? 'No tienes acceso a ningún stream con esta cuenta' : 'Acceso restringido al equipo autorizado'}
          </p>
        </div>
        ...
        {sinAcceso ? (
          <>
            <p className="text-[12px] text-[#999] text-center leading-relaxed">
              La cuenta <span className="text-white">{email}</span> no está invitada a ningún stream.
              Pídele al equipo que te comparta uno, o entra con otra cuenta.
            </p>
            <button onClick={onSalir}
              className="w-full px-4 py-2.5 bg-white text-gray-800 text-[13px] font-medium rounded-xl hover:bg-gray-50 transition-colors">
